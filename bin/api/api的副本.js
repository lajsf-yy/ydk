...
class ApiCodeGenerator {
  ...
  addTypeDefinition(name, definition, baseClass) {
    let properties = {};
    let required = definition.required || [];
    let matchGenerics = false;
    baseClass && console.warn('baseClass', baseClass, name);
    ...
    for (let propertyName in definition.properties) {
      ...
      try {
        ...
        if (baseClass && (tsType === baseClass || tsType == `${baseClass}[]`)) {
          console.warn('matchGenerics ', baseClass);
          matchGenerics = true;
    ...
    if (matchGenerics || !this.typeDefinitionMap.has(name)) {
  ...
  getRefType(refType) {
    ...
    let isGenerics = tsType.indexOf('<') > -1;
    if (definition) {
      if (isGenerics) {
        let genericsClass = tsType.substr(0, tsType.indexOf('<')) + '<T>';
        let baseClass = tsType.substr(tsType.indexOf('<') + 1);
        baseClass = baseClass.substr(0, baseClass.lastIndexOf('>'));
        this.addTypeDefinition(genericsClass, definition, baseClass);
      } else {
