...
public class PicturePreviewActivity extends PictureBaseActivity implements
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.yui_picture_preview);
        if (!RxBus.getDefault().isRegistered(this)) {
            RxBus.getDefault().register(this);
        }
        mHandler = new Handler();
        screenWidth = ScreenUtils.getScreenWidth(this);
        animation = OptAnimationLoader.loadAnimation(this, R.anim.yui_modal_in);
        ...
        tv_ok.setText(numComplete ? getString(R.string.yui_picture_done_front_num,
            ...
            public void onClick(View view) {
                if (images != null && images.size() > 0) {
                    ...
                    if (!TextUtils.isEmpty(pictureType)) {
                        ...
                        if (!toEqual) {
                            ToastManage.s(mContext,getString(R.string.yui_picture_rule));
                            return;
                        }
                    }
                    // 刷新图片列表中图片状态
                    ...
                    if (selectImages.size() >= config.maxSelectNum && isChecked) {
                        ToastManage.s(mContext, getString(R.string.yui_picture_message_max_num, config.maxSelectNum));
                        check.setSelected(false);
                        return;
                    }
                    if (isChecked) {
    ...
    public void onSelectNumChange(boolean isRefresh) {
        ...
        if (enable) {
            ...
            if (numComplete) {
                tv_ok.setText(getString(R.string.yui_picture_done_front_num, selectImages.size(),
                ...
                tv_ok.setText(getString(R.string.yui_picture_completed));
            }
        } else {
            id_ll_ok.setEnabled(false);
            tv_ok.setSelected(false);
            if (numComplete) {
                tv_ok.setText(getString(R.string.yui_picture_done_front_num, 0,
                        config.selectionMode == PictureConfig.SINGLE ? 1 : config.maxSelectNum));
            } else {
                tv_img_num.setVisibility(View.INVISIBLE);
                tv_ok.setText(getString(R.string.yui_picture_please_select));
            }
        }
