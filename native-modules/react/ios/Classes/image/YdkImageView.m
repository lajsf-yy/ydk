...
@implementation YdkImageView
{
  ...
  SDWebImageCombinedOperation *_reloadImageCancellationBlock;
...
{
  SDWebImageCombinedOperation* previousCancellationBlock = _reloadImageCancellationBlock;
  if (previousCancellationBlock) {
    [previousCancellationBlock cancel];
...
- (void)reloadImage
{
  ...
  if (source && self.frame.size.width > 0 && self.frame.size.height > 0) {
    ...
    __weak YdkImageView *weakSelf = self;
    ...
    if (!UIEdgeInsetsEqualToEdgeInsets(_capInsets, UIEdgeInsetsZero)) {
      ...
      SDInternalCompletionBlock completionHandler = ^(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error, SDImageCacheType cacheType, BOOL finished, NSURL * _Nullable imageURL) {
          [weakSelf imageLoaderLoadedImage:image error:error forImageSource:source partial:NO];
      };
      
      SDWebImageOptions options = SDWebImageRetryFailed;
      _reloadImageCancellationBlock=   [[SDWebImageManager sharedManager] loadImageWithURL:source.request.URL
                                                                                   options:options
