...
NSString * YdkToastPositionTop       = @"YdkToastPositionTop";
NSString * YdkToastPositionCenter    = @"YdkToastPositionCenter";
NSString * YdkToastPositionBottom    = @"YdkToastPositionBottom";

// Keys for values associated with toast views
static const NSString * YdkToastTimerKey             = @"YdkToastTimerKey";
static const NSString * YdkToastDurationKey          = @"YdkToastDurationKey";
static const NSString * YdkToastPositionKey          = @"YdkToastPositionKey";
static const NSString * YdkToastCompletionKey        = @"YdkToastCompletionKey";

// Keys for values associated with self
static const NSString * YdkToastActiveKey            = @"YdkToastActiveKey";
static const NSString * YdkToastActivityViewKey      = @"YdkToastActivityViewKey";
static const NSString * YdkToastQueueKey             = @"YdkToastQueueKey";

@interface UIView (toast_private)
...
- (void)makeToast:(NSString *)message {
    [self makeToast:message duration:[YdkToastManager defaultDuration] position:[YdkToastManager defaultPosition] style:nil];
}
...
- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(id)position style:(YdkToastStyle *)style {
    UIView *toast = [self toastViewForMessage:message title:nil image:nil style:style];
    [self showToast:toast duration:duration position:position completion:nil];
}

- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(id)position title:(NSString *)title image:(UIImage *)image style:(YdkToastStyle *)style completion:(void(^)(BOOL didTap))completion {
...
- (void)showToast:(UIView *)toast {
    [self showToast:toast duration:[YdkToastManager defaultDuration] position:[YdkToastManager defaultPosition] completion:nil];
}
...
- (void)showToast:(UIView *)toast duration:(NSTimeInterval)duration position:(id)position completion:(void(^)(BOOL didTap))completion {
    ...
    objc_setAssociatedObject(toast, &YdkToastCompletionKey, completion, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    
    if ([YdkToastManager isQueueEnabled] && [self.cs_activeToasts count] > 0) {
        // we're about to queue this toast view so we need to store the duration and position as well
        objc_setAssociatedObject(toast, &YdkToastDurationKey, @(duration), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        objc_setAssociatedObject(toast, &YdkToastPositionKey, position, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)hideToast:(UIView *)toast {
    ...
    NSTimer *timer = (NSTimer *)objc_getAssociatedObject(toast, &YdkToastTimerKey);
    [timer invalidate];
...
- (void)cs_showToast:(UIView *)toast duration:(NSTimeInterval)duration position:(id)position {
    ...
    if ([YdkToastManager isTapToDismissEnabled]) {
    ...
    [UIView animateWithDuration:[[YdkToastManager sharedStyle] fadeDuration]
                          delay:0.0
                     ...
                     animations:^{
                         ...
                         objc_setAssociatedObject(toast, &YdkToastTimerKey, timer, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
                     }];
}
...
- (void)cs_hideToast:(UIView *)toast fromTap:(BOOL)fromTap {
    [UIView animateWithDuration:[[YdkToastManager sharedStyle] fadeDuration]
                     ...
                     animations:^{
                         ...
                         void (^completion)(BOOL didTap) = objc_getAssociatedObject(toast, &YdkToastCompletionKey);
                         if (completion) {
                             completion(fromTap);
                         }
                         ...
                         if ([self.cs_toastQueue count] > 0) {
                             ...
                             NSTimeInterval duration = [objc_getAssociatedObject(nextToast, &YdkToastDurationKey) doubleValue];
                             id position = objc_getAssociatedObject(nextToast, &YdkToastPositionKey);
...
- (UIView *)toastViewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image style:(YdkToastStyle *)style {
    // sanity
    if(message == nil && title == nil && image == nil) return nil;
    
    // default to the shared style
    if (style == nil) {
        style = [YdkToastManager sharedStyle];
    }
...
- (NSMutableArray *)cs_activeToasts {
    NSMutableArray *cs_activeToasts = objc_getAssociatedObject(self, &YdkToastActiveKey);
    if (cs_activeToasts == nil) {
        cs_activeToasts = [[NSMutableArray alloc] init];
        objc_setAssociatedObject(self, &YdkToastActiveKey, cs_activeToasts, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    return cs_activeToasts;
}

- (NSMutableArray *)cs_toastQueue {
    NSMutableArray *cs_toastQueue = objc_getAssociatedObject(self, &YdkToastQueueKey);
    if (cs_toastQueue == nil) {
        cs_toastQueue = [[NSMutableArray alloc] init];
        objc_setAssociatedObject(self, &YdkToastQueueKey, cs_toastQueue, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
...
- (void)makeToastActivity:(id)position {
    // sanity
    UIView *existingActivityView = (UIView *)objc_getAssociatedObject(self, &YdkToastActivityViewKey);
    if (existingActivityView != nil) return;
    
    YdkToastStyle *style = [YdkToastManager sharedStyle];
    ...
    objc_setAssociatedObject (self, &YdkToastActivityViewKey, activityView, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    ...
    if (existingActivityView != nil) {
        [UIView animateWithDuration:[[YdkToastManager sharedStyle] fadeDuration]
                         ...
                         animations:^{
                             ...
                             objc_setAssociatedObject (self, &YdkToastActivityViewKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
                         }];
    }
}
...
- (CGPoint)cs_centerPointForPosition:(id)point withToast:(UIView *)toast {
    ...
    if([point isKindOfClass:[NSString class]]) {
        if([point caseInsensitiveCompare:YdkToastPositionTop] == NSOrderedSame) {
            return CGPointMake(self.bounds.size.width/2, (toast.frame.size.height / 2) + style.verticalPadding);
        } else if([point caseInsensitiveCompare:YdkToastPositionCenter] == NSOrderedSame) {
...
@interface YdkToastManager ()

@property (strong, nonatomic) YdkToastStyle *sharedStyle;
...
@implementation YdkToastManager

#pragma mark - Constructors

+ (instancetype)sharedManager {
    static YdkToastManager *_sharedManager = nil;
...
- (instancetype)init {
    self = [super init];
    if (self) {
        self.sharedStyle = [[YdkToastStyle alloc] initWithDefaultStyle];
        self.tapToDismissEnabled = YES;
        self.queueEnabled = YES;
        self.defaultDuration = 3.0;
        self.defaultPosition = YdkToastPositionCenter;
    }
    return self;
}
